...
(function () {
  ...
  function toggleWidget(config, widgetConfig) {
    ...
    if (iframe.style.display === 'none') {
      ...
      const bubbleIcon = widgetConfig.theme?.bubbleIcon || widgetConfig.brand?.bubbleIcon;
      const isVoice = widgetConfig.type === 'voice';

      if (bubbleIcon && (bubbleIcon.startsWith('http') || bubbleIcon.startsWith('data:'))) {
        bubble.innerHTML = `<img src="${bubbleIcon}" alt="Chat" style="width: 32px; height: 32px; object-fit: contain;">`;
      } else if (isVoice) {
