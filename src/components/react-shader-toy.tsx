...
export function ReactShaderToy({
  ...
  const setUniforms = (timestamp: number) => {
    ...
    if (uniformsRef.current.iFrame?.isNeeded) {
      const frameUniform = gl.getUniformLocation(shaderProgramRef.current, UNIFORM_FRAME);
      const currentFrame = (uniformsRef.current.iFrame.value as number) || 0;
      gl.uniform1i(frameUniform, currentFrame);
      uniformsRef.current.iFrame.value = currentFrame + 1;
    }
