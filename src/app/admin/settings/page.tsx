...
export default function SettingsPage() {
  return (
    <div className="flex flex-col gap-4 p-8">
      ...
      <p className="text-lg text-muted-foreground font-medium">Configure global system parameters, API integrations, and workspace preferences.</p>
        ...
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-64 rounded-[32px] bg-muted/30 shadow-sm flex flex-col items-center justify-center p-8 text-center gap-4">
            ...
            <div className="space-y-2">
              <h3 className="font-bold text-foreground/50">Core Configuration</h3>
              <p className="text-sm text-muted-foreground/40">Securing environment variables and system protocols.</p>
            </div>
          </div>
        ))}
      </div>
